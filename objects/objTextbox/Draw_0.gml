confirmkey = keyboard_check_pressed(ord("Z")) or keyboard_check_pressed(vk_space);
skip_key = keyboard_check_pressed(ord("C"));
textbox_x = camera_get_view_x(view_camera[0]);
textbox_y = camera_get_view_y(view_camera[0]) + 160;

// setup

if setup == false {
	setup = true;
	objAstral.can_move = false;
	draw_set_font(Dialoguefont);
	draw_set_valign(fa_top);
	draw_set_halign(fa_left);

// loop through the page
	page_number = array_length(text);
	for (var p = 0 ; p < page_number ; p++ ) {
		// get amount of characters per page
		text_length[p] = string_length(text[p]);
		
		// get x position for textbox
		text_x_offset = 32;
		
		
	}

}

// typing the text

if draw_char < text_length[page] {
	draw_char += text_speed;
	draw_char = clamp(draw_char, 0, text_length[page]);
}

// flip thru pages
if confirmkey {
	// if typing is done
	if draw_char == text_length[page] {
		// next page
		if page < page_number - 1 {
			page++;
			draw_char = 0;
		} else {
			// destroy textbox
			instance_destroy();
			objAstral.can_move = true;
		}
	} else if skip_key and draw_char != text_length[page] {
	draw_char = text_length[page]; }
}

// draw the textbox
txtb_spr_w = sprite_get_width(txtb_sprite);
txtb_spr_h = sprite_get_height(txtb_sprite);

// back of box
draw_sprite_ext(txtb_sprite, txtb_image, textbox_x + text_x_offset, textbox_y, textbox_width/txtb_spr_w, textbox_height/txtb_spr_h, 0, c_white, 1);

// draw text
var _drawtext = string_copy(text[page], 1, draw_char);
draw_text_ext(textbox_x + text_x_offset + border, textbox_y + border, _drawtext, line_sep, line_width);